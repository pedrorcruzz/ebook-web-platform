    ...
    <div class="container mx-auto px-4" style="min-height: 90vh;">
        ...
        <div class="flex justify-center mt-8">
            <form method="GET" action="{{ route('app.index') }}" class="w-full max-w-md flex">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Pesquisar livros..."
                    class="flex-1 p-2 border border-gray-300 rounded-l-md">
                <button type="submit"
                    class="bg-indigo-600 text-white px-4 rounded-r-md hover:bg-indigo-700">Buscar</button>
            </form>
        </div>
                ...
                <div class="bg-white shadow-md rounded-lg overflow-hidden">
                    ...
                    <div class="p-4">
                        ...
                        <a href="{{ route('app.books.details', ['id' => $livro->id]) }}"
                            class="text-blue-500 hover:underline mt-2 block">Colocar no Carrinho</a>
                    </div>
                </div>
            @empty
